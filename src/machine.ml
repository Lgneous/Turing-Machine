...
let is_valid_blank desc =
  if List.mem desc.blank desc.alphabet then Result.Ok desc
  else Result.Bad "Blank character is not in alphabet."

let is_valid_initial desc =
  if List.mem desc.initial desc.states then Result.Ok desc
  else Result.Bad "Initial state is not in state list."

let is_valid_finals desc =
  if List.for_all (is_in desc.states) desc.finals then Result.Ok desc
  else Result.Bad "Final states must be a sublist of state list."

let is_valid_transitions desc =
  if Enum.for_all (is_in desc.states) (Hashtbl.keys desc.transitions) then
    Result.Ok desc
...
let is_valid_read desc =
  ...
  if Enum.for_all _is_valid_read @@ Enum.map _reads values then Result.Ok desc
...
let is_valid_to_state desc =
  ...
  if Enum.for_all _is_valid_to_state @@ Enum.map _to_state values then
    Result.Ok desc
...
let is_valid_write desc =
  ...
  if Enum.for_all _is_valid_write @@ Enum.map _writes values then
    Result.Ok desc
...
let sanitize desc =
  ...
  >>= is_valid_transitions >>= is_valid_read >>= is_valid_to_state
  >>= is_valid_write

let print_running_transition state t =
  print_endline @@ " (" ^ state ^ ", "
  ^ String.make 1 t.Transition.read
  ^ ") -> (" ^ t.Transition.to_state ^ ", "
  ^ String.make 1 t.Transition.write
  ^ ", "
  ^ (if t.Transition.action = Transition.Left then "LEFT" else "RIGHT")
  ^ ")"
...
let rec run desc tape state =
  ...
  try
    let transi =
      Transition.from_read (Hashtbl.find desc.transitions state) read_v
    in
    match transi with
    | Result.Bad e -> if is_in desc.finals state then "OK" else e
    | Result.Ok t ->
        print_running_transition state t ;
        run desc
          ( tape
          |> flip Tape.write t.Transition.write
          |> flip Tape.shift t.Transition.action )
          t.Transition.to_state

...
let rec print_transition = function
    | (name, [])    -> ()
    | (name, t::tl) -> print_endline @@ "(" ^ name ^ ", " ^ t.Transition.read ^ ") -> ("
                     ^ t.Transition.to_state ^ ", " ^ t.Transition.write ^ ", "
                     ^ begin if t.Transition.action = Left then "LEFT" else "RIGHT" end
...
let machine_desc desc =
    ...
    print_endline @@ "*" ^ (String.repeat " " 79) ^ "*";
    print_endline @@ String.repeat "*" 81;
    print_string "Alphabet: [ "; print_list desc.Machine.alphabet; print_endline " ]";

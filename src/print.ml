...
let rec print_transitions states transitions =
  ...
  | [] -> ()
  | s :: tl -> (
    match Hashtbl.find_option transitions s with
    | None -> print_transitions tl transitions
    | Some lst ->
        print_transition (s, lst) ;
        print_transitions tl transitions )
...
let machine_desc desc =
  ...
  ^ desc.Machine.name
  ^ String.repeat " " (39 - (String.length desc.Machine.name / 2))
  ^ if String.length desc.Machine.name mod 2 = 1 then "*" else " *" ;
  print_endline @@ "*" ^ String.repeat " " 79 ^ "*" ;
  print_endline @@ String.repeat "*" 81 ;
  print_string "Alphabet: [ " ;
  print_list @@ List.map (String.make 1) desc.Machine.alphabet ;
  print_endline " ]" ;
  print_string "States : [ " ;
  print_list desc.Machine.states ;
  print_endline " ]" ;
  print_endline @@ "Initial : " ^ desc.Machine.initial ;
  print_string "Finals  : [ " ;
  print_list desc.Machine.finals ;
  print_endline " ]" ;
